...
#PLACEHOLDER insert vm_name into table_jobid                                                                                                                               
...
EOF
    ...
    if [ -n "$latest_checkpoint" ]; then
      ...
      log_events "Starting Incremental Backup with checkpoint: $latest_checkpoint"
...
EOF
        ...
        log_events "Backup initialization started for VM ID: $vm_id_input"
      else
        sqlite3 "$db_file" <<EOF
...
EOF
      ...
      log_events "Checking if backup is ready..."
      #/root/spinner1.sh 30 
      sleep 30
      ...
      log_events "Checkpoint ID: $init_status_checkpoint"
...
EOF
        ...
        log_events "Backup ready for VM ID: $vm_id_input"
      else
        echo "Backup is not ready yet. Waiting..."
        log_trace "Backup is not ready yet. Waiting..."
        log_events "Backup is not ready yet. Waiting..."
        sleep 30
      fi
    else
...
log_events "Created incremental table in database: incremental_$sanitized_vm_id"
...
EOF
      echo "Updating Checkpoint in incremental table..."
       log_trace "Updating Checkpoint in incremental table..."
       log_trace "Updating Checkpoint in incremental table..."
...
log_events "Image Transfer Start Response: $imageTransfer_start"
...
log_trace "Fetching download URL for transfer ID: $imageTransfer_transferID"
log_events "Fetching download URL for transfer ID: $imageTransfer_transferID"
...
log_trace "Processing extents..."
log_events "Processing extents.."
    ...
    log_events "Download completed at Date: $fixed_date, Time: $fixed_time"
...
fetch_backup_info_and_update() {
    ...
    log_events "Fetching backup information for VM ID: $vm_id_input"
...
EOF
  ...
  # Fetch the TIME and DATE values from the incremental table
  incremental_time_date=$(sqlite3 "$backup_index_db" <<EOF
SELECT Time, Date FROM incremental_$sanitized_vm_id ORDER BY Date DESC, Time DESC LIMIT 1;
EOF
  )

  IFS="|" read -r incremental_time incremental_date <<< "$incremental_time_date"

  # Echo the fetched TIME and DATE values
  echo "Fetched TIME from incremental table: $incremental_time"
  echo "Fetched DATE from incremental table: $incremental_date"
  log_trace "Fetched TIME from incremental table: $incremental_time"
  log_events "Fetched DATE from incremental table: $incremental_date"

  global_timestamp_time=$incremental_time
  global_timestamp_date=$incremental_date
...
VALUES ('$vm_id_input', '$fetchvmattribs_vmName', '$fetchvmattribs_diskID', $Full_Backup, '$backup_files', '$fetchvmattribs_checkpoint', '$global_timestamp_time', '$global_timestamp_date', '$fetchvmattribs_status', '$total_size_mb');
EOF
...
# Function to calculate deduplication
calculate_deduplication() {
    echo " "
    echo "#######################################"
    echo "# Calculating Deduplication Ratio     #"
    echo "#######################################"
    echo " "

    # Get the current checkpoint
    current_checkpoint=$(sqlite3 "$backup_index_db" "SELECT Checkpoint FROM incremental_$sanitized_vm_id ORDER BY Date DESC, Time DESC LIMIT 1;")
    
    if [ -z "$current_checkpoint" ]; then
        echo "No checkpoint found, cannot calculate deduplication ratio."
        return
    fi

    # Echo the current checkpoint
    echo "Current Checkpoint: $current_checkpoint"

    # Calculate the total length for the current checkpoint
    total_length=$(sqlite3 "$backup_index_db" "SELECT IFNULL(SUM(Length), 0) FROM incremental_$sanitized_vm_id WHERE Checkpoint='$current_checkpoint';")

    # Calculate the total zero length for the current checkpoint
    total_zero_length=$(sqlite3 "$backup_index_db" "SELECT IFNULL(SUM(Length), 0) FROM incremental_$sanitized_vm_id WHERE Checkpoint='$current_checkpoint' AND Zero='true';")

    echo "Total length of all extents: ${total_length} bytes"
    echo "Total zero length: ${total_zero_length} bytes"

    # Retrieve the incremental file paths
    incremental_file_paths=$(sqlite3 "$backup_index_db" "SELECT Backup_Path FROM table_BI_$sanitized_vm_id WHERE Full_Backup=0;")

    # Initialize total size
    total_size_bytes=0
    
    # Process each line of file paths
    while IFS= read -r line; do
        IFS=',' read -ra file_list <<< "$line"
        for file_path in "${file_list[@]}"; do
            file_path=$(echo "$file_path" | tr -d '\r' | tr -d ' ' | sed 's/,$//g')
    ...
    done <<< "$incremental_file_paths"
    
    total_size_mb=$(awk "BEGIN {printf \"%.2f\", ${total_size_bytes}/(1024*1024)}")

    echo "Total size of all incremental files: ${total_size_mb} MB"

    # Calculate Zero Block Deduplication Ratio as a percentage
    if [ "$total_size_bytes" -gt 0 ]; then
        deduplication_ratio=$(awk "BEGIN {printf \"%.2f\", (($total_length - $total_zero_length) / $total_size_bytes) * 100}")
        
        # Ensure the ratio is capped at 100% if it exceeds that value
        if (( $(echo "$deduplication_ratio > 100" | bc -l) )); then
            deduplication_ratio=100
        fi

        echo "Zero Block Deduplication Ratio: $deduplication_ratio%"

        # Calculate and display savings
        savings=$(awk "BEGIN {printf \"%.2f\", ${total_length} / ${total_size_bytes} - 1}")
        echo "Savings due to deduplication: $savings MB"
        percentage_savings=$(awk "BEGIN {printf \"%.2f\", (1 - ${total_size_bytes}/${total_length}) * 100}")
        echo "Percentage Savings: $percentage_savings%"

        # Save results to a file
        mkdir -p "/backup/status/$sanitized_vm_id/ZDP"
        echo "Zero Block Deduplication Ratio: $deduplication_ratio%" > "/backup/status/$sanitized_vm_id/ZDP/ZDP_${current_checkpoint}.txt"
        echo "Savings due to deduplication: $savings MB" >> "/backup/status/$sanitized_vm_id/ZDP/ZDP_${current_checkpoint}.txt"
        echo "Percentage Savings: $percentage_savings%" >> "/backup/status/$sanitized_vm_id/ZDP/ZDP_${current_checkpoint}.txt"

    else
        echo "Total size of incremental files is zero, cannot calculate deduplication ratio."
    fi
}

# Call the deduplication calculation function
calculate_deduplication
...
log_trace " Running ASPAR"
log_events "Running ASPAR"

/root/myenv/bin/python  /root/entropy_analysis.py $vm_id_input
                               
                                                                                                     
log_trace " CBT Delta for $vm_name with VM ID: $vm_id_input  COMPLETED "                                               
