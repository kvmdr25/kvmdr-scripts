...
EOF
    ...
    if [ -n "$latest_checkpoint" ]; then
      ...
      echo " init start status"
      echo $init_start_query
      echo " "
     # Check VM disk status
     disk_status=$(curl -s -k -H "Authorization: Bearer $bearer_Token" "$url_Engine/api/vms/$vm_id_input/disks")
...
EOF
  ...
  log_trace "Fetched DATE from incremental table: $incremental_date"
