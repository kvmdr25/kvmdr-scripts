...
echo "#########################"
echo " "
echo "<--Branch for DR Simulator Commit to Recovery---->"
...
selected_rewind_date="$2"        # Restore Date (YYYY-MM-DD)
selected_rewind_time="$3"        # Restore Time (HH:MM:SS)

# Ensure all 3 arguments are provided
while [[ -z "$vm_id_input" || -z "$selected_rewind_date" || -z "$selected_rewind_time" ]]; do
    echo "ERROR: Missing required arguments."
    echo "Usage: $0 <VMID> <YYYY-MM-DD> <HH:MM:SS>"
    
    # Ask again for inputs
    if [[ -z "$vm_id_input" ]]; then
        read -p "Enter VM ID: " vm_id_input
    fi
    if [[ -z "$selected_rewind_date" ]]; then
        read -p "Enter Restore Date (YYYY-MM-DD): " selected_rewind_date
    fi
    if [[ -z "$selected_rewind_time" ]]; then
        read -p "Enter Restore Time (HH:MM:SS): " selected_rewind_time
    fi
done
...
DRurl_Engine="https://dr.local/ovirt-engine"                                                                                                                                          
...
echo "${timestamp}" > "/tmp/timestamp_commitrecovery${vm_id_input}.txt"
...
log_trace " Recovering from Simulator "
log_events " Recovering from Simulator "
...
log_trace " Remove Cron Scheduler "
log_events " Remove Cron Scheduler "
...
log_trace " All tokens Fetched  "
log_events " All tokens Fetched "

# Function to perform Synthetic Backup
create_synthetic() {
  echo "Starting Create Synthethic Backup process..."

log_trace " Create Synthethic Backup process...merging"
log_events "Create Synthethic Backup process...merging"
  ...
  new_backup_file="/backup/Full_backup_${vm_name}_RecoveredFO_${selected_rewind_date}_${selected_rewind_time}.raw"

log_trace " Copy and rename the merged backup file "
log_events "Copy and rename the merged backup file "
...
log_trace " Making sure correct checkpoint copied for first Incremental in new table "
log_events " Making sure correct checkpoint copied for first Incremental in new table  "
  ...
  response_checkpoint=$(sqlite3 "$backup_index_db" "SELECT rewind_time, rewind_date FROM drsimulator_$sanitized_vm_id WHERE new_vmid='$vm_id_input' LIMIT 1;")
     ...
     # Split the response into rewind_date and rewind_time
        ...
        # Ensure user-provided values are used
           rewind_time="$selected_rewind_time"
           rewind_date="$selected_rewind_date"
  ...
  #checkpoint=$(sqlite3 "$backup_index_db" "SELECT Checkpoint FROM table_BI_$sanitized_original_vmid WHERE Date='$selected_rewind_date' AND Time<='$selected_rewind_time' ORDER BY Date DESC, Time DESC LIMIT 1;")
  checkpoint=$(sqlite3 "$backup_index_db" "SELECT Checkpoint FROM table_BI_$sanitized_original_vmid WHERE (Date < '$selected_rewind_date') OR (Date = '$selected_rewind_date' AND Time <= '$selected_rewind_time') ORDER BY Date DESC, Time DESC LIMIT 1;")
  echo " "
...
log_trace " Get the Simulator vm name "
log_events " Get the Simulator vm name  "
...
log_trace " Starting checkpoint migration..long process "
log_events "  Starting checkpoint migration.. long process"
...
EOF
  ...
  echo " "
log_trace " Checkpoint Migration Process completed successfully! "
log_events "  Checkpoint Migration Process completed successfully!"
}
...
log_trace " Run incremental backup after reverse replication "
log_events " Run incremental backup after reverse replication"

# Run incremental backup after reverse replication
echo "Running incremental backup after reverse replication..."
/root/incremental_fofbdr_v21.sh $vm_id_input simulator-now 
...
log_trace " Create and populate the sourceFO table "
log_events " Create and populate the sourceFO table"
