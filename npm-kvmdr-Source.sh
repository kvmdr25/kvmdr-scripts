...
logfile="$logdir/npm_Source_$timestamp.log"
...
if [ -e "$logdir/npm_latest.log" ]; then
    ...
    rm -f "$logdir/npm_Source_latest.log"
fi
...
ln -sf "$logfile" "$logdir/npm_Source_latest.log"

# Update the npm_start in the Source table
sqlite3 /root/vmSettings.db <<EOF
UPDATE Source
SET npm_start = '$timestamp'
WHERE id = 1;
EOF
