...
app.get("/backup/HE/:host", (req, res) => {
  ...
  const scriptPath = "/root/HE_backup_v3.sh";
  const command = `${scriptPath} ${host}`;
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    return res.json({ success: true, message: `Backup initiated for ${host}`, log: stdout });
  });
});

// Update he_protection_enabled flag in Source or Target table
app.post("/update-he-protection", (req, res) => {
  const { hostType, enabled } = req.body;
  ...
  db.run(
    `UPDATE ${table} SET he_protection_enabled = ? WHERE id = 1`,
    [value],
    function(err) {
      db.close();
      if (err) {
      ...
      console.log(`[INFO] Updated HE Protection: ${hostType.toUpperCase()} -> ${enabled ? "ENABLED" : "DISABLED"}`);
      return res.json({ success: true, message: "HE Protection setting updated successfully." });
    }
  );
});
    ...
    async (err, backup) => {
      ...
      try {
        ...
        if (selected_files.includes("answers") || selected_files.includes("hosted-engine")) {
          ...
          logMessage(eventLog, "[COPY] answers.conf and hosted-engine.conf copied");
        }
        ...
        if (selected_files.includes("ova") && backup.ova_file) {
          ...
          logMessage(eventLog, "[COPY] OVA copied");
        }
        ...
        if (selected_files.includes("engine-db") && backup.engine_backup_file && await fse.pathExists(backup.engine_backup_file)) {
          ...
          logMessage(traceLog, "[COPY] Engine DB backup copied");
          logMessage(eventLog, "[COPY] Engine DB backup copied");
        }
        ...
        if (selected_files.includes("dom_md") && backup.dom_md_path && await fse.pathExists(backup.dom_md_path)) {
          ...
          logMessage(eventLog, "[COPY] dom_md directory copied");
        }
